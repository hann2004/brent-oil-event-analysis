...
def get_event_correlation():
    ...
    try:
                ...
                correlations.append(
                    {
                        ...
                        "date": event["Date"].strftime("%Y-%m-%d"),
                        "type": event["Event_Type"],
                        "price_correlation": impact["price_impact"]["percent_change"]
                        / 100,  # Convert to correlation-like metric
                        "volatility_correlation": impact["volatility_impact"][
                            "percent_change"
                        ]
                        / 100,
                        "expected_direction": event.get(
                            "Expected_Price_Direction", "neutral"
                        ),
                        "actual_direction": "positive"
                        if impact["price_impact"]["percent_change"] > 0
                        else "negative",
                    }
                )
        ...
        if correlations:
            avg_price_corr = sum(c["price_correlation"] for c in correlations) / len(
                correlations
            )
            direction_match = (
                sum(
                    ...
                    if c["expected_direction"] == c["actual_direction"]
                )
                / len(correlations)
                * 100
            )
