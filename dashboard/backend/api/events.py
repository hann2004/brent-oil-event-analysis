        ...
        return jsonify(
            {
                ...
                "metadata": {
                    ...
                    "available_types": list(
                        data_loader.df_events["Event_Type"].unique()
                    ),
                    "date_range": {
...
def get_event_impact():
    ...
    try:
        ...
        if event_title and not event_date:
            event = data_loader.df_events[
                data_loader.df_events["Event_Title"] == event_title
            ]
            if len(event) == 0:
                return (
                    jsonify(
                        {"success": False, "error": f"Event not found: {event_title}"}
                    ),
                    404,
                )
        ...
        event_details = data_loader.df_events[
            data_loader.df_events["Date"] == event_date_dt
        ]
