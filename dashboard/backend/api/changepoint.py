    ...
    try:
        ...
        return jsonify({
            ...
            "data": {
                ...
                "model_metrics": data_loader.changepoint_results.get('model_metrics', {})
            }
        })
...
def get_changepoint_impact(cp_date):
    ...
    try:
        ...
        if not cp_data:
            return jsonify({
                "success": False,
                "error": f"Change point not found: {cp_date}"
            }), 404
        ...
        if len(before_data) == 0 or len(after_data) == 0:
            return jsonify({
                "success": False,
                "error": "Insufficient data for impact analysis"
            }), 400
        ...
        return jsonify({
            ...
            "data": {
                ...
                "impact": {
                    ...
                    "price_change_pct": ((after_stats['mean_price'] - before_stats['mean_price']) / before_stats['mean_price']) * 100,
                    "volatility_change": after_stats['std_price'] - before_stats['std_price'],
                    "volatility_change_pct": ((after_stats['std_price'] - before_stats['std_price']) / before_stats['std_price']) * 100
                },
                "statistics": {
                    "before": before_stats,
                    "after": after_stats
